...
router.post('/', authRequired, async (req, res) => {
    try {
        const item = await MediaLogo.create(req.body);
...
router.put('/:id', authRequired, async (req, res) => {
    try {
        const item = await MediaLogo.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
router.delete('/:id', authRequired, async (req, res) => {
    try {
        const r = await MediaLogo.findByIdAndDelete(req.params.id);
